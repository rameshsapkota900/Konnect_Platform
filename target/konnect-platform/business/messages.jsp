...
<body>
    ...
    <main>
        ...
        <section class="about-section">
            <div class="about-card">
                ...
                <div style="display: grid; grid-template-columns: 300px 1fr; gap: 1.5rem; height: 600px; max-height: 70vh; background-color: var(--white); border-radius: 10px; box-shadow: var(--shadow); overflow: hidden;">
                    <div style="border-right: 1px solid var(--light-gray); height: 100%; display: flex; flex-direction: column;">
                        <div style="padding: 1rem; border-bottom: 1px solid var(--light-gray); background-color: var(--light-gray);">
                            <h3 style="margin: 0; font-size: 1.1rem; color: var(--secondary-color);">Conversations</h3>
                        </div>
                        ...
                        <div style="flex-grow: 1; overflow-y: auto; padding: 0.5rem;">
                            <% if(conversationPartners == null || conversationPartners.isEmpty()) { %>
                                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 2rem; text-align: center; color: var(--secondary-color);">
                                    ...
                                    <p style="font-size: 0.9rem;">Start messaging creators to collaborate on campaigns.</p>
                                </div>
                            <% } else { %>
                                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                    <% for(User conversationPartner : conversationPartners) { %>
                                        <a href="<%= request.getContextPath() %>/messages?userId=<%= conversationPartner.getId() %>" style="display: flex; align-items: center; gap: 1rem; padding: 0.8rem; border-radius: 8px; text-decoration: none; color: var(--text-color); background-color: <%= partner != null && partner.getId() == conversationPartner.getId() ? "var(--light-gray)" : "transparent" %>; transition: all 0.2s;">
                                            <div style="width: 40px; height: 40px; border-radius: 50%; background-color: <%= partner != null && partner.getId() == conversationPartner.getId() ? "var(--primary-color)" : "var(--secondary-color)" %>; color: white; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 1rem;">
                                                <%= conversationPartner.getUsername().substring(0, 1).toUpperCase() %>
                                            </div>
                                            <div style="flex-grow: 1; overflow: hidden;">
                                                <div style="font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><%= conversationPartner.getUsername() %></div>
                                                <div style="font-size: 0.8rem; color: var(--secondary-color); text-transform: capitalize;"><%= conversationPartner.getRole() %></div>
                                            </div>
                                        </a>
                                    <% } %>
                                </div>
                    ...
                    <div style="display: flex; flex-direction: column; height: 100%;">
                        ...
                        <% } else { %>
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 2rem; text-align: center; color: var(--secondary-color);">
                                ...
                                <p>Select a conversation from the list or start a new one by messaging a creator.</p>
                                <a href="<%= request.getContextPath() %>/business/creators" class="btn btn-primary" style="margin-top: 1.5rem; display: inline-flex; align-items: center; gap: 0.5rem;">
    ...
    <script>
        ...
        window.onload = function() {
            var messagesList = document.querySelector('[style*="flex-grow: 1; overflow-y: auto; padding: 1.5rem;"]');
            if (messagesList) {
