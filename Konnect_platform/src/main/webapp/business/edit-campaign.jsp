...
<jsp:include page="/includes/header.jsp">
    <jsp:param name="title" value="Edit Campaign" />
</jsp:include>
            ...
            <div class="dashboard">
                ...
                <div class="dashboard-content">
                    <div class="dashboard-header">
                        <h2 class="dashboard-title"><i class="fas fa-edit"></i> Edit Campaign</h2>
                    ...
                    <div class="card">
                        <form action="<%= request.getContextPath() %>/campaign/edit/<%= campaign.getCampaignId() %>" method="post" class="form">
                            <div class="form-group">
                                <label for="title">Campaign Title <span class="required">*</span></label>
                                <input type="text" id="title" name="title" class="form-control" value="<%= campaign.getTitle() %>" required>
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description" name="description" class="form-control" rows="5" required><%= campaign.getDescription() %></textarea>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="number" id="budget" name="budget" class="form-control" min="1" step="0.01" value="<%= campaign.getBudget() %>" required>
                                    <div class="form-text">Enter your campaign budget in USD.</div>
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="goals">Campaign Goals <span class="required">*</span></label>
                                    <input type="text" id="goals" name="goals" class="form-control" value="<%= campaign.getGoals() %>" required>
                                    <div class="form-text">What are your goals for this campaign? (e.g., Increase brand awareness)</div>
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="status">Campaign Status <span class="required">*</span></label>
                                <select id="status" name="status" class="form-control" required>
                                    <option value="active" <%= campaign.getStatus().equals("active") ? "selected" : "" %>>Active</option>
                                    <option value="completed" <%= campaign.getStatus().equals("completed") ? "selected" : "" %>>Completed</option>
                                    <option value="cancelled" <%= campaign.getStatus().equals("cancelled") ? "selected" : "" %>>Cancelled</option>
                                </select>
                                <div class="form-text">Update the status of your campaign.</div>
                            </div>

                            <div class="form-actions">
                                <button type="submit" class="btn btn-primary btn-fixed-height "><i class="fas fa-save"></i> Update Campaign</button>
                                <a href="<%= request.getContextPath() %>/campaign" class="btn btn-outline">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
